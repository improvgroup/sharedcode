...
public static partial class CollectionExtensions
{
    ...
    /// implement <see cref="ICollection{T}"/> and <see cref="IEnumerable{T}"/> where <typeparamref
    /// name="T"/> is <see cref="KeyValuePair{TKey, TValue}"/>.
    /// </summary>
         ...
         Func<T, TKey> getKey)
    {
        ...
        foreach (var item in @this.Where(x => !keep.Contains(getKey(x))))
        {
            _ = @this.Remove(item);
         ...
         Func<TKey, T> getObject)
    {
        ...
        foreach (var item in @this
            .Where(x => !keep.Contains(getKey(x)))
            .ToArray())
        {
            _ = @this.Remove(item);
            returnValue.Removed.Add(item);
    ...
    public static bool TrueForAll<T>(this ICollection<T> @this, Predicate<T> match)
    {
        ...
        return @this.All(item => match(item));
    }
}
