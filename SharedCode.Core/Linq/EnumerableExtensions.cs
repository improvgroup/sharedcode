...
public static class EnumerableExtensions
{
	...
	public static IOrderedEnumerable<TSource> OrderBy<TSource, TKey>(this IEnumerable<TSource> @this, Func<TSource, TKey> keySelector, bool descending)
	{
		...
		return (descending ? @this?.OrderByDescending(keySelector) : @this?.OrderBy(keySelector))
#if NET6_0_OR_GREATER
			?? Enumerable.Empty<TSource>().Order();
#else
			?? Enumerable.Empty<TSource>().OrderBy(i => i);
#endif
	}
		...
		params Func<TSource, IComparable>[] keySelectors)
	{
		...
		{
#if NET6_0_OR_GREATER
			return Enumerable.Empty<TSource>().Order();
#else
	...
	public static IOrderedEnumerable<T> Randomize<T>(this IEnumerable<T> @this) => (@this ?? []).OrderBy(_ => Random.Next());
	...
	public static IEnumerable<IEnumerable<T>> Transpose<T>(this IEnumerable<IEnumerable<T>> @this)
	{
		while (true)
		{
			...
			var xss = @this.Skip(1);
			var xssList = xss as IList<IEnumerable<T>> ?? [.. xss];
			return new[]
			{
				new[] {val}.Concat(
