...
/// </summary>
[SuppressMessage("Usage", "GCop500:Void async methods cannot be awaited. Also they can hide exceptions and cause tricky bugs. Return a Task instead.", Justification = "The whole point is to fire and forget without await.")]
public static class ValueTaskExtensions
{
    /// <summary>
    /// Safely execute the <see cref="ValueTask"/> without waiting for it to complete before moving
    ...
    /// If an exception is thrown in the <see cref="ValueTask"/>, <c>onException</c> will execute.
    ...
    /// <summary>
    /// Safely execute the <see cref="ValueTask{TResult}"/> without waiting for it to complete
    /// before moving to the next line of code; commonly known as "Fire And Forget".
    /// </summary>
    /// <typeparam name="TException">The type of exception.</typeparam>
    /// <typeparam name="TResult">The result type.</typeparam>
    /// <param name="task">The <see cref="ValueTask{TResult}"/>.</param>
    /// <param name="continueOnCapturedContext">Whether to continue on captured context.</param>
    /// <param name="onException">Optional exception handler.</param>
    public static async void SafeFireAndForgetAsync<TException, TResult>(
    ...
    /// <param name="continueOnCapturedContext">Whether to continue on captured context.</param>
    /// <param name="onException">Optional exception handler.</param>
    public static async void SafeFireAndForgetAsync<TResult>(
        this ValueTask<TResult> task,
        bool continueOnCapturedContext = true,
        Action<Exception>? onException = null)
    {
