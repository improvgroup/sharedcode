...
/// </summary>
/// <typeparam name="TMutationIdentityKey">The type of the key for the identity performing mutations on the entities handled by this context.</typeparam>
...
public abstract class AuditableDbContext<TMutationIdentityKey> : DbContext where TMutationIdentityKey : notnull
{
	...
	/// <summary>Gets the identity performing the mutation of the entities.</summary>
	/// <returns>The mutation identity.</returns>
	/// <example>
	///     <code title="Sample implementation.">this.httpContextAccessor?.HttpContext?.User?.Identity?.Name ?? "MyApp";</code>
	///     <code title="Don't forget to add the accessor to your dependency container in your Program.cs or Startup.cs file.">services.AddHttpContextAccessor();</code>
	/// </example>
